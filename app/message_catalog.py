...
ACHIEVEMENT_REASONS = {
    ...
    "nothing": ("何もしていないという事実を正直に告白しました", "今日はまだ、力を温存しています", "これから何かをする可能性を残しています"),
}

MOOD_REASONS = {
    "tired": ("心身に疲労が見られます", "休息と塩分を求めているようです"),
    "hungry": ("空腹という大切な告白があります", "胃袋が静かに訴えています"),
...
FORGIVENESS_REASONS = {
    ...
    "strict": ("反省は必要ですが、ラーメンとの両立は可能です", "ラーメン欲が強いようですが、まだ赦しの余地もあります"),
...
REASON_NOT_TO_EAT_REASONS = {
    "none": ("食べない理由はなく、赦しを妨げるものもありません", "止める理由がないなら、残る問題は店選びだけです"),
    "yes": ("ためらう理由があるため、量と体調には気を配ってください", "食べない理由も尊重し、無茶をしないことが条件です"),
    "ignore": ("都合の悪い理由から目をそらした点は、少しだけ懺悔してください", "食べない理由を無視するなら、少なくとも水は飲みなさい"),
}
