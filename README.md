...
フロントエンド処理は`app/static/script.js`を入口とするES Modulesへ分割しています。

- `app/static/js/clock.js`: 現在日時と統計リンク
- `app/static/js/reveal.js`: スクロール表示演出
- `app/static/js/question_messages.js`: サーバー生成の質問設定読み込み
- `app/static/js/interview_flow.js`: 診断質問フローとシスターのリアクション
- `app/static/js/certificate_canvas.js`: 免罪符Canvas画像生成
- `app/static/js/download_certificate.js`: 画像ダウンロード操作
- `app/static/js/page_enhancements.js`: カウントアップ、自動送信、ボタン演出
