...
ローカルまたはRender Shellで、`RAMEN_DB_PATH`が指すSQLite DBを`backups/`へバックアップできます。環境変数が未設定の場合は`data/ramen.db`を使用します。
...
作成されたバックアップは、次のコマンドでSQLiteの整合性と必須テーブルを確認します。

```bash
python scripts/verify_sqlite_backup.py backups/ramen-YYYYMMDD-HHMMSS.db
```

検証では`diagnosis_logs`と`standalone_judgments`を実際に読み取り、行数を表示します。`backups/*.db`はGit管理対象外です。Render Persistent Disk上へバックアップを残す場合は、`--backup-dir /app/data/backups`を指定してください。
