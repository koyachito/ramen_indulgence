...
- `DATABASE_URL`未設定時のSQLiteは、ホスティング環境の再デプロイや再起動でデータが消える可能性があります。
- PostgreSQL接続コードはありますが、本番移行、バックアップ、障害復旧の運用手順は未整備です。
...
- 公開環境のDB方針を確定し、PostgreSQL移行、バックアップ、障害時対応を整備する
